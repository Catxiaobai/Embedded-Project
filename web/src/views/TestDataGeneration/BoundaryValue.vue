  ...
  <div id="fullState">
    <el-card>
      ...
      >
        ...
        <el-table-column prop="time" label="边界精度" width="100">
          <template slot-scope="scope">
            <el-input class="tableCell" autosize v-model="scope.row.precision"> </el-input>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="114">
          <template slot-scope="scope">
            <el-button size="mini" @click="generateBoundary(scope.$index, scope.row)" type="primary">边界值生成</el-button>
          </template>
        </el-table-column>
    ...
    GenerateAllData() {
      ...
      for (let i = 0; i < this.percentTotal; i++) {
        ...
        setTimeout(() => {
          ...
          //设置延迟执行
          this.$http.post(this.Global_Api + '/api/generation/generate_boundary', this.rawData[i]).then((response) => {
            console.log(response.data)
          })
        }, 3000 * i)
      }
