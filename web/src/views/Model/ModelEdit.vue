  ...
  <div id="modelEdit">
    <el-card>
      <div id="action" style="display: flex; margin-bottom: 20px">
        <!--        <el-button type="primary" @click="loadTxt">loadTxt</el-button>-->
        <div class="block">
          <el-cascader v-model="value" :options="options" placeholder="查看变量"></el-cascader>
        </div>
        <el-button type="primary" style="margin-left: 80%" @click="save">save</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    load() {
      let model = go.Model.fromJson(this.modelData)
      this.myDiagram.model = model
    },
    save() {
      console.log(this.myDiagram.model.toJson())
      this.$http.post(this.Global_Api + '/api/save_model_to_txt', this.myDiagram.model.toJson()).then((response) => {
    ...
    loadTxt() {
      this.$http.post(this.Global_Api + '/api/load_txt', this.itemInfo).then((response) => {
        if (response.data.error_code === 0) {
          console.log(response)
    ...
    set_options() {
      this.$http.post(this.Global_Api + '/api/model_variable_options', this.itemInfo).then((response) => {
        if (response.data.error_code === 0) {
          console.log(response)
          // this.$message.success('成功')
